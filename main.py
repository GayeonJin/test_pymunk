...
def test_gravity() :
    ...
    space.add(pymunk.Segment(space.static_body, (sx+100, sy+80), (ex-100, ey-80), boarder_width))
...
def test_split1() :
    ...
    def coll_separate(arbiter, space, data) :
        ...
        shape.body.velocity = (vel[0] / 2, 0)

        object = ball(pos, 10, 20)
        object.set_velociy(-vel[0] / 2, 0)
        space.add(object.body, object.shape)
    ...
    coll_handler = space.add_collision_handler(3, 1)
    coll_handler.separate = coll_separate

    coll_handler1 = space.add_collision_handler(3, 2)
    coll_handler1.begin = coll_begin
    ...
    while running:
        for event in pygame.event.get() :
            ...
            if event.type == pygame.MOUSEBUTTONUP :
                ...
                if l_button == True :
                    object = bullet(mouse_pos)
                    object.set_velociy(0, -1000)
                    space.add(object.body, object.shape)
